...
// Structure to store the hashes in
...
//hash function, keeps indexes within the hash table
int hash(int key) {
    int hash = key % SIZE;
    return  hash < 0 ? hash+SIZE : hash;
}

//insert a number into the hashTable
void insert(HashTable *hashTable, int *keyTable, int number, int index) {
    int x, realIndex = number;
    //find the first available spot in the table if there is a collision
    while(x = hash(number), keyTable[x])  
        number = (number + 1) % SIZE;
	
    hashTable[x].hashIndex = index;
    if(realIndex != number) {   //if collision occured, set realIndex and set collision value
        hashTable[x].realIndex = realIndex;
        hashTable[x].collision = 1;
    }
    keyTable[x] = 1;
}

int search(HashTable *hashTable, int *keyTable, int number, int index) {
    int x = hash(number), num = number;
    
    //return first index if there is no collision
    if(hashTable[x].collision == 0 && keyTable[x] == 1)
        return hashTable[hash(number)].hashIndex;
    
    //if there is a collision, loop until number is found or not found
    while(x = hash(number), keyTable[x] && hashTable[x].collision) {
        if(hashTable[x].realIndex == num)
	    return hashTable[x].hashIndex;
        number = (number + 1) % SIZE;
    }
    return -SIZE;
}
...
int *twoSum(int *nums, int numsSize, int target, int *returnSize) {
    int *retarray = calloc(sizeof(int),2);
    HashTable *hashTable = calloc(sizeof(HashTable),SIZE);
    int *keyTable = calloc(sizeof(int),SIZE);
    int num, ret;
    //loop through the nums, store into hash table
    for(int i = 0; i < numsSize; i++) {
        num = target - nums[i]; //take the different from the target num, check if the difference num is in hash table
	ret = search(hashTable, keyTable, num, i);
        
        //if num is found in hash table, then set return array
	if(ret >= 0) {
	    *returnSize = 2; retarray[0] = ret; retarray[1] = i;	
            free(hashTable); free(keyTable);
            break;
	}
	insert(hashTable, keyTable, nums[i], i);
    }
    return retarray;
}
